...
public final class SHA384 {
  ...
  /**
   * 对输入的 bytes 进行 sha384 散列
   */
  public static byte[] digest(byte[] input) throws NoSuchAlgorithmException {
    return Digests.digest(input, DigestAlgorithm.SHA384, null);
  }

  /**
   * 对输入的 bytes 进行 sha384 散列，带 salt 达到更高的安全性
   */
  public static byte[] digest(byte[] input, byte[] salt) throws NoSuchAlgorithmException {
    return Digests.digest(input, DigestAlgorithm.SHA384, salt);
  }
  ...
  public static byte[] digest(String input) throws NoSuchAlgorithmException {
    return Digests.digest(input.getBytes(StandardCharsets.UTF_8), DigestAlgorithm.SHA384, null);
  }
  ...
  public static byte[] digest(String input, Charset charset) throws NoSuchAlgorithmException {
    return Digests.digest(input.getBytes(charset), DigestAlgorithm.SHA384, null);
  }

  /**
   * 对输入的字符串进行 sha384 散列，带 salt 达到更高的安全性
   */
  public static byte[] digest(String input, byte[] salt) throws NoSuchAlgorithmException {
    return Digests.digest(input.getBytes(StandardCharsets.UTF_8), DigestAlgorithm.SHA384, salt);
  }
  ...
  public static byte[] digest(String input, Charset charset, byte[] salt) throws NoSuchAlgorithmException {
    return Digests.digest(input.getBytes(charset), DigestAlgorithm.SHA384, salt);
  }
  ...
  public static byte[] digest(InputStream inputStream) throws NoSuchAlgorithmException, IOException {
    return Digests.digest(inputStream, DigestAlgorithm.SHA384);
  }
